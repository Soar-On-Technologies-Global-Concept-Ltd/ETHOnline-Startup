...
_llm: AnthropicLLM | FakeLLM | None = None


def llm() -> AnthropicLLM | FakeLLM:
    global _llm
    if _llm is None:
        _llm = FakeLLM() if get_settings().llm_provider == "fake" else AnthropicLLM()
    return _llm


def set_llm(instance: AnthropicLLM | FakeLLM | None) -> None:
    global _llm
    _llm = instance
