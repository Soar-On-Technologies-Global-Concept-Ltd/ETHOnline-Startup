...
async def release(tx_id: uuid.UUID, body: ChainReportIn, request: Request, user: CurrentUser = Depends(current_user)):
    async def execute(s, _):
        tx = await lock(s, tx_id)
        _, role = await load_transaction(s, tx_id, user)
        require("customer", role)
        ...
        return await payments.release_call(s, tx, user.id)
